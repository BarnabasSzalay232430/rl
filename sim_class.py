...
class Simulation:
    def __init__(self, num_agents, render=True, rgb_array=False, model_path=None):
        ...
        if self.cv_model:
            self.root_coordinates = run_pipeline(self.plate_image_path, self.cv_model, plate_position_robot = [0.10775, 0.062, 0.17])
            logging.info(f"Extracted root coordinates: {self.root_coordinates}")
        else:
    ...
    def get_root_coordinates(self):
        """Return the extracted root coordinates."""
