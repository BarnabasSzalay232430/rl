...
# Constants
PLATE_SIZE_MM = 150  # Physical size of the petri dish in millimeters
PLATE_POSITION_ROBOT = [0.10775, 0.062, 0.17]  # Base position of the plate in robot coordinates
RECT_POSITIONS = [
    # Normalized positions of rectangular regions of interest within the plate
...
def f1_metric(y_true, y_pred, threshold=0.3):
    """
    Calculate the F1 metric for evaluating segmentation performance.
    ...
    positives = tf.reduce_sum(tf.round(tf.clip_by_value(y_true, 0, 1)))  # Actual Positives
    pred_positives = tf.reduce_sum(tf.round(tf.clip_by_value(y_pred, 0, 1)))  # Predicted Positives
...
def combined_loss(y_true, y_pred):
    """
    Combined loss function using Dice loss and Binary Cross Entropy (BCE).
...
def extract_petri_dish(image):
    """
    Extract the petri dish region from the image using contour detection.

    Args:
        image (np.ndarray): Grayscale input image.

    Returns:
        np.ndarray: Cropped region containing the petri dish.
    """
...
def predict_root_mask(image, model, patch_size=128, stride=64):
    """
    Predict the root mask using a deep learning model.

    Args:
        image (np.ndarray): Input grayscale image.
        model: Trained TensorFlow model for segmentation.
        patch_size (int): Size of patches used for prediction.
        stride (int): Stride for sliding window patch extraction.

    Returns:
        np.ndarray: Binary mask of predicted roots.
    """
    h, w = image.shape
    patches = []
...
def connect_roots(mask):
    """
    Connect fragmented roots in the binary mask using morphological operations.

    Args:
        mask (np.ndarray): Binary mask of segmented roots.

    Returns:
        np.ndarray: Dilated binary mask with connected roots.
    """
...
def filter_and_skeletonize_roots(mask, rect_positions, min_area=200):
    """
    Filter roots based on rectangular regions of interest, limit to largest objects, and skeletonize them.

    Args:
        mask (np.ndarray): Binary mask of connected roots.
        rect_positions (list): Normalized positions of regions of interest.
        min_area (int): Minimum area for a root to be considered.

    Returns:
        tuple: Filtered mask, skeletonized mask, root data, and labels.
    """
    h, w = mask.shape
...
def find_root_tips_and_convert_to_robot_coordinates(
    ...
    """
    Identify root tips, convert their coordinates to robot coordinates, annotate the image, and save it.

    Args:
        skeletonized_mask (np.ndarray): Skeletonized mask of roots.
        root_data (list): Filtered root objects and IDs.
        labels (np.ndarray): Connected component labels.
        conversion_factor (float): Pixel-to-mm conversion factor.
        petri_dish (np.ndarray): Cropped petri dish image.
        plate_position_robot (list): Base robot position for the plate.
        output_path (str): Path to save the annotated image.

    Returns:
        list: List of root tip coordinates in robot space.
    """
    ...
    for rect_id, obj_id in root_data:
        ...
        x_robot = y_mm / 1000 + plate_position_robot[0]
        y_robot = x_mm / 1000 + plate_position_robot[1]
...
def run_pipeline(image_path, model, plate_position_robot, output_dir=r"C:\Users\szala\Documents\GitHub\rl2\highlighted_roottips"):
    """
    Full pipeline to process an image, predict root tips, and convert coordinates for robotic use.
